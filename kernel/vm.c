  ...
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    ...
    if(PTE_FLAGS(*pte) == PTE_V)
      ...
      if((*pte & PTE_V) == 0){
        ...
        else{ // if PG flag is on, do not kfree (it is in swapFile)!
              //Delete from swapfile and continue to next 
          struct proc * p = myproc();
...
uvminit(pagetable_t pagetable, uchar *src, uint sz)
{
  ...
  void swapOut(struct proc * p){
    ...
    *pte = *pte & ~PTE_V; //turning off the PTE_V flag
    ...
    #ifdef NFUA
      p->physPagesArray[pagetableIndexToSwap].counter = 0;
    #endif
  ...
  swapIn(struct proc * p, uint64 va){
    ...
    // walk to the inner level of va in the pagetable, getting the most inner pte
    pte_t* pte = walk(p->pagetable, va, 0); // to do make sure va is ok

    //allocate a page in ram mem
    mem = kalloc();
    if(mem == 0){
      return;
    }
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  for(a = oldsz; a < newsz; a += PGSIZE){
    #ifndef NONE
      // if so add what possible to the physical memmory 
      // for making space to the other pages - swapp these amount of pages (other pages)
