...
//
//system call ment to allow the user to get the number of pagefaults a process has done.
//in addition, when sending num>=0 it sets the process' numOfPageFault field to be 0
//
int
    ...
    //}
      ...
      for(int k = 0; k < MAX_PSYC_PAGES; k++){
        ...
        np->physPagesArray[k].va = p->physPagesArray[k].va; 
          ...
          np->swapPagesArray[k].counter = p->swapPagesArray[k].counter;
          np->physPagesArray[k].counter = p->physPagesArray[k].counter;
        #endif
      ...
      #if defined(NFUA) || defined(LAPA) 
        np->swapPagesArray[16].counter = p->swapPagesArray[16].counter;
      #endif
...
sched(void)
{
  //printf(".\n"); //for debugging algo reasons and specifically keeping track of pte_a flag
  int intena;
  struct proc *p = myproc();
